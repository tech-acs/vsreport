...
We will be using data provided by the [2021 Nigeria Multiple Indicators Cluster Survey (MICS)](9https://mics.unicef.org/sites/mics/files/Nigeria%20MICS%202021%20Statistical%20Snapshots_English.pdf) on the percentage of children under age 5 whose births are registered by each Nigerian state.
...
We will also being using the [sf](https://cran.r-project.org/web/packages/sf/index.html) package to load spatial data, [RColorBrewer](https://cran.r-project.org/web/packages/RColorBrewer/index.html) for colour palettes and [dplyr](https://cran.r-project.org/web/packages/dplyr/index.html) for data manipulation.
...
Administrative data typically come in vector format (points, lines, and polygons) as an [ESRI shapefile](https://doc.arcgis.com/en/arcgis-online/reference/shapefiles.htm) (.shp) or a [GeoJSON](https://geojson.org/) (.geojson, .json). The `st_read` function from the `sf` package can read either file format.

Here we are loading a GeoJSON of all the states of Nigeria. The `sf` package converts the GeoJSON file into a simple features object, a data frame with features stored in rows and attributes in columns. The feature geometries of the object are stored in a list-column at the end. These are the coordinates that describe the boundary of each state.
