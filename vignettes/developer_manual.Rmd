...
to improve the package. First, install the `vsreport` from GitHub,
then ensure all package dependencies are correctly managed and installed using `renv`:

### 1) Clone the repository:

```sh
git clone https://github.com/tech-acs/vsreport.git
cd vsreport
```

### 2) Initialize the `renv` environment: Upon first entering the package directory,
`renv` will activate the specific package environment defined for `vsreport`.
...
dependencies and the correct versions installed, allowing `vsreport`
...
┣ 📜vsreport.Rproj           # AUTOMATED set-up for R Project
