...
n <- 1000
id <- sample(1:n)
birth1a <- as.character(sample(seq(as.Date("2017/01/01"), as.Date("2024/01/01"), by = "day"), n, replace = TRUE)) # coerce to character
birth1b <- sample(seq(as.Date("2017/01/01"), as.Date("2024/01/01"), by = "day"), n, replace = TRUE)
...
birth1i <- sample(c("Hospital", "Home", "Clinic", "Other", "Unknown", NA), n, replace = TRUE)
birth2a <- sample(c("M", "F"), n, replace = TRUE)
birth3b <- sample(0:90, n, replace = TRUE)
birth3b[c(3, 5, 7)] <- NA # add some missing values

df <- data.frame(id, birth1a, birth1b, birth1c, birth1g, birth1h, birth1i, birth2a, birth3b)
df <- rbind(df, df[rep(1, 10), ]) # duplicate rows
rownames(df) <- NULL
...
The variable `birth1b` is of type date, `birth3b` integer, and the others are all character.
...
There may be inputting errors in the civil registration data such as the date of occurrence being recorded *after* the date of registration. To identify those records that are out of sync we can use a simple filter. The code below selects all of the rows where the date of occurrence is after the date of registration and counts them.
...
Sometimes the order of levels in a factor variable is not very helpful. The [`fct_relevel()`](https://forcats.tidyverse.org/reference/fct_relevel.html) function allows you to customise the order of levels. Here we reorder the values of `birth1g` (Type of birth) so that "Quadruplet or higher" comes last.
```{r}
df$birth1g |> levels()
...
Sometimes values of a variable are coded unhelpfully. For example, the sex of a newborn baby may be recorded as "M" and "F" or 0 and 1.
```{r}
df$birth2a |> levels()
```

We can easily recode these so that they are more readable. The [`fct_recode()`](https://forcats.tidyverse.org/reference/fct_recode.html) function enables you to change factor levels by hand.
```{r}
df$birth2a |> 
...
## Missing data
Missing values are recorded in a variety of ways e.g. 999999, NULL, or even "". For example, the place of occurrence might not be recorded and left blank. We can recode these missing values to a more explicit NA using the [`na_if()`](https://dplyr.tidyverse.org/reference/na_if.html) function.
```{r}
df <- mutate(df, birth1c = na_if(birth1c, ""))
```

To identify how many missing values there are in each column we can run:
```{r}
df |> map_dbl(~sum(is.na(.)))
```

There are `r sum(is.na(df$birth1c))` NA values in `birth1c`, `r sum(is.na(df$birth1i))` in `birth1i`, and `r sum(is.na(df$birth3b))` in `birth3b`. 

In cases where there are missing values in an integer variable we can impute them as an average. For example, there are `r sum(is.na(df$birth3b))` missing values in `birth3b` which is the age of the mother. We can impute the mean age with:

```{r}
df <- df |> 
  mutate(birth3b = case_when(is.na(birth3b) ~ as.integer(mean(birth3b, na.rm = TRUE)), TRUE ~ birth3b))
```
