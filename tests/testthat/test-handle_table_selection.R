...
# Unit Tests
test_that("handle_table_selection returns correct table IDs for a given section number", {
  all_tables <- list(
  ...
  result <- handle_table_selection("3", all_tables)
  expected <- list("3_1", "3_2")
  expect_equal(result, expected)
})

test_that("handle_table_selection returns empty list when no table IDs match the section number", {
  all_tables <- list(
    list(table_id = "3_1", table_name = "Summary Table", function_name = "create_t3.1"),
    list(table_id = "3_2", table_name = "Detailed Table", function_name = "create_t3.2_t3.3"),
    list(table_id = "4_1", table_name = "Summary Table", function_name = "create_t4.1"),
  ...
  result <- handle_table_selection("6", all_tables)
  expected <- list()
  expect_equal(result, expected)
})
