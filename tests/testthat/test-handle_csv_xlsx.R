...
test_that("handle_csv_xlsx creates an Excel file with the correct sheets", {
  ...
  # Run the handle_csv_xlsx function
  handle_csv_xlsx(input_path = temp_dir, output_path = output_file)
...
test_that("handle_csv_xlsx warns if no CSV files are found", {
  # Create an empty temporary directory
  empty_dir <- tempdir()

  # Expect a warning when no CSV files are found
  expect_warning(handle_csv_xlsx(input_path = empty_dir, output_path = file.path(empty_dir, "output.xlsx")), "There are no .csv files in this directory")
})
